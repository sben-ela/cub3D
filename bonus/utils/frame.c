...
/*   Updated: 2023/05/28 16:25:09 by sben-ela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	__moves(t_data *data, int direction, double x, double y)
{
	...
	if (data->map[(int)(data->player.y)]
		[(int)(data->player.x + 3 * x)] != '1'
		&& data->map[(int)(data->player.y)]
		[(int)(data->player.x + 3 * x)] != 'D')
		data->player.x += x;
	if (data->map[(int)(data->player.y + 3 * y)]
		[(int)(data->player.x)] != '1'
	&& data->map[(int)(data->player.y + 3 * y)]
		[(int)(data->player.x)] != 'D')
		data->player.y += y;
}
...
void	moves(t_data *data)
{
	...
	if (data->hooks.rotation != -1)
	{
		...
		rotate(&data->player.plane_x, &data->player.plane_y,
			direction * ROT_ANGLE);
	}
	_moves(data, 0);
}
...
int	frame(t_data *data)
{
	...
	draw_rays(data);
	draw_minimap(data, 0, 0, 0);
	mlx_put_image_to_window(data->mlx, data->win, data->img.img, 0, 0);
	if (data->reload == 1)
	{
		mlx_put_image_to_window(data->mlx, data->win,
			data->weapon[i % 84], -80 + data->left_right, 280);
		if (i % 84 == 0)
			data->reload = -1;
		i++;
	}
		...
		mlx_put_image_to_window(data->mlx, data->win,
			data->weapon[0], -80 + data->left_right, 280);
